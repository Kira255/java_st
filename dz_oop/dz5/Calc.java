...
public class Calc {
    ...
    public String result(String exp) throws CalcException {
        ...
        // ---проверка, что создалось 3 элемента: число1, оператор, число2, иначе
        // исключение
        if (expItems.size() != 3) {
            throw new CalcException(
                    "ОШИБКА. Выражение должно иметь вид: \"Число1 Операция Число2\", разделенные пробелом...");
        }
        ...
        if (parse.isNumeric(expItems.get(0)) && parse.isNumeric(expItems.get(2))) { // ---проверяем, что оба числа
                                                                                    // арабские
            num1 = Integer.parseInt(expItems.get(0));
            num2 = Integer.parseInt(expItems.get(2));
            isRomanExp = false;
        } else if (parse.isRoman(expItems.get(0)) && parse.isRoman(expItems.get(2))) { // ---проверяем, что оба числа
                                                                                       // римские
